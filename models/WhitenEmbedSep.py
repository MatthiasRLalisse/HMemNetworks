...
class WhitenEmbedSep(KBEModel):
	"""Class to whiten embeddings (ZCA transform), separating Relation & Entity vectors"""
	...
	def whiteningTransform(self, Embeddings, embeddings_, alpha=.2, normalize=True):
		...
		print('centered embed shape', E_centered.shape)
	...
	def build_probes(self, model):
		...
		e1_embed, e2_embed, e_mem_embed = \
			self.whiteningTransform(model.e_embeddings_, whiten_entities)
		r, r_mem_embed = \
			self.whiteningTransform(model.r_embeddings_, whiten_relations)

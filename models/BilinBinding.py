...
class BilinBinding(KBEModel):
	...
	def __init__(self,
		...
		if not name:
			name = 'BilinBinding%ieD%irD%sL-%s' % (entity_dim, relation_dim, str(lambda_) \
	...
	def unbind_op(self, Mem_h, r, e):	#0 is left direction, 1 is right, 
		...
		self.WUBind = pinv(WBind_)
		#print(Mem_h.shape, WUBind.shape)
		tpr_ravel = tf.einsum('kh,bh->bk', self.WUBind, Mem_h)
		#print(tpr_ravel.shape)
		...
		#print('the tpr shape', mu_M_tpr.shape)

...
def read_results(fname):
	try:
		with open(fname, 'r') as f:
			lines = [ [ s.split() for s in l.split('\t') if len(s.split()) > 1 ] for l in f.readlines() ]
		results = defaultdict(list)
		for line in lines:
			try:
				for key, val in line:
			  		results[key].append(eval(val))
			except: pass
		for rez in results:
			if rez != 'epoch':
				results[rez] = [x for _, x in \
					sorted(zip(results['epoch'], results[rez]))]
	except FileNotFoundError:
		keys = ['epoch', 'MR', 'MRR', 'Hits1', 'Hits3', 'Hits10']

...
#from .embeddings import WhitenEmbed	#factors the whitening ops
...
data_presets = defaultFalseDict({'freebase':path+'/data/FB15K', 'wordnet':path+'/data/WN18', \
					...
					'wn18rr_r': path+'/data/WN18RR_recip',
					'fb237_r': path+'/data/FB15K-237_recip', })	
...
class KBEModel(object):
	...
	def build_embeddings(self):
		with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
			self.e_embeddings_ = tf.get_variable("entityEmbeddings", 
				shape=[self.n_entity, self.entity_dim], dtype=tf.float32)
			self.r_embeddings_ = tf.get_variable("relationEmbeddings", 
				shape=[self.n_relation*2, self.relation_dim], dtype=tf.float32)
	...
	def build_Mem_embeddings(self):
		...
		#	self.r_mem_embed = tf.nn.dropout(self.r_mem_embed, rate=self.train_dropout)	
		#	self.b_metric = tf.nn.dropout(self.b_metric, keep_prob=1.-self.drop_prob)
		#	self.e_mem_embed = tf.nn.dropout(self.e_mem_embed, rate=self.train_dropout)	
		#	self.b_metric =
	
	def build_Mem(self):
		self.W_relations = tf.get_variable("W_relations", 
				shape=[self.relation_dim+self.entity_dim, \
				self.relation_dim+self.entity_dim], dtype=tf.float32)
	...
	def build_params(self):	
		with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
			self.W_ = tf.get_variable("W_", shape=[self.h_dim, self.h_dim], dtype=tf.float32)
			#self.W__ = tf.
			...
			self.W_filter = tf.get_variable("W_filter", shape=[self.h_dim, self.h_dim], dtype=tf.float32)
			self.b_filter = tf.get_variable("b_filter", shape=[self.h_dim], dtype=tf.float32)
			...
			b = tf.get_variable("b_", shape=[self.h_dim, self.entity_dim], dtype=tf.float32)
			if self.train_dropout != 0:
	...
	def build_classifier(self, trip=True):
		...
		self.W_class = tf.get_variable('W_class', shape=[self.n_types, self.h_dim], dtype=tf.float32)
		self.b_class = tf.get_variable("b_class", shape=[self.n_types], dtype=tf.float32)
	...
	def save(self, force_overwrite=False):
		...
		#model_files = [ f for f in os.listdir(self.model_dir) if re.match(model_id, f) ]
		...
		#results_ = [ e: self.results[(e,d)]['MRR'] for e, d in self.results if d == 'valid' ]
		#best_epoch = max(results_, key=lambda x: results_[x])
		#delete_files = [ f for f in os.listdir(model_path) 
